...
App.Views.EditMaterialFormContainerView = Backbone.View.extend({
  ...
  materials_labels: null, // справочник лейблов материалов
  /**
   * Инициализация
   */
  initialize: function()
  {
    // справочник лейблов материалов
    this.materials_labels = this.options.materials_labels;
    // все данные
    ...
    this.AddMaterialView = new App.Views.EditMaterialView({
      model:new App.Models.MaterialItemModel({},{'parse':true}),
      Materials_Units: this.Materials_Units,
      sectors:this.sectors,
      materials_groups: this.materials_groups,
      edit: true,
      materials_labels: this.materials_labels
    });
    ...
    if(material_key){
      ...
      $.ajax({
        ...
        async:true
          ...
          {
            // форма редактирования материала
            self.EditMaterialView = new App.Views.EditMaterialView({
              model:new App.Models.MaterialItemModel(result['data'],{parse: true}),
              Materials_Units: self.Materials_Units,
              sectors:self.sectors,
              materials_groups: self.materials_groups,
              edit: true,
              materials_labels: self.materials_labels
            });

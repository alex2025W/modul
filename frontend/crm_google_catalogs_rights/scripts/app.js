...
App.Views.AppView = Backbone.View.extend({
  ...
  onSaveData: function(e)
  {
    ...
    // collect data
    data.order_numbers = Routine.removeAllSpaces(this.$('.tb-order-numbers').val())?Routine.removeAllSpaces(this.$('.tb-order-numbers').val()).split(';'):[];
    data.catalogs = Routine.removeAllSpaces(this.$('.tb-catalogs').val())?Routine.removeAllSpaces(this.$('.tb-catalogs').val()).split(';'):[];
    data.users = Routine.removeAllSpaces(this.$('.tb-users').val())?Routine.removeAllSpaces(this.$('.tb-users').val()).split(';'):[];
    data.access = this.$('.ddl-access').val();
    // check data
    if(data.order_numbers.length===0)
    {
      $.jGrowl('Не заданы номера заявок', { 'themeState':'growl-error', 'sticky':false, life: 10000 });
      return;
    }
    if(data.catalogs.length===0)
    {
      $.jGrowl('Не заданы каталоги', { 'themeState':'growl-error', 'sticky':false, life: 10000 });
      return;
    }
    if(data.users.length===0)
    {
      $.jGrowl('Не заданы пользователи', { 'themeState':'growl-error', 'sticky':false, life: 10000 });
      return;
    }
    if(data.access==='')
    {
      $.jGrowl('Не задан тип доступа', { 'themeState':'growl-error', 'sticky':false, life: 10000 });
      return;
    }
    ...
    this.queue = new Queue({
        task_key: "set_crm_google_catalogs_rights",
        params: data,

...
var App = {
    ...
    DataWorks: [],
    DataMaterials: [],
    Materials_Units: {}, // список единиц измерения, задействованный во всех материалах {"unit_pto":[], "unit_purchase":[], "sku_name":[]'}
    /**
    ...
    **/
    initialize: function(data)
    {
        ...
        for(var group_index in data.materials_groups)
        {
            ...
            if(material_group_info['materials'] && material_group_info['materials'].length>0)
            {
        ...
        this.EditMaterialFormContainerView = new App.Views.EditMaterialFormContainerView({'data':this.Data, 'Materials_Units':this.Materials_Units});
        // слушатель события изменения данных о матери
        Backbone.on("global:on_material_save_complete",this.onMaterialUpdateComplete,this);
    },
    ...
    **/
    onMaterialUpdateComplete: function(e){
        ...
        if(type=="add")
        {
            ...
            {
                if(this.Data.materials_groups[group_index]['code'] == material_info['group_code']){
            ...
            App.DataMaterials.sort(function (a, b) {
                    if(a.group_name == b.group_name)
                        return (a.material_name < b.material_name) ? -1 : (a.material_name > b.material_name) ? 1 : 0;
                    else
            ...
            for(var i in App.DataMaterials){
                ...
                if(tmp_material['material_id'].toString() == material_info['_id'].toString())
                {
            ...
            for(var group_index in this.Data.materials_groups)
            {
                if(this.Data.materials_groups[group_index]['code'] == material_info['group_code']){
                    ...
                    for(var mi in material_group_info['materials']){
                        ...
                        if(tmp_material['_id'].toString() == material_info['_id'].toString())
                        {
        ...
        this.ContainerView.RenderMaterials(this.ContainerView.$el.find('.btn-collapse-materials').val()=='unCollapsed', this.ContainerView.$el.find('.btn-show').val() == 'show', (this.ContainerView.cur_work_item)?this.ContainerView.cur_work_item['work_materials']:null);
...
App.Views.ContainerView = Backbone.View.extend({
    ...
    **/
    ...
    {
        this.$el.find('.works-data-body').html(this.templates.works_data(App.DataWorks));
    },
    ...
    **/
    RenderMaterials: function(is_collapsed, show_only_checked, materials)
    {
        ...
        this.$el.find('.materials-data-body').html(this.templates.materials_data(data_materials));
        ...
        if(materials && materials.length>0)
            for(var m in materials)
            {
                ...
                var cur_material_item =this.$el.find('.cb-material[data-id="'+material_id+'"]')
                $(cur_material_item).prop('checked', true);

                var container = $(cur_material_item).parents("ul:first").parents("li:first");
                // если все материалы в группе выбраны, то и группу надо поместить как выбранную
                var all_materials_checked = $(container).find('.cb-material:checked').length>0;
                $(container).find('.cb-material').each(function(i){
                    if(!$(this).prop('checked'))
                        all_materials_checked  = false;
                });
                $(container).find('.cb-material-all').prop('checked', all_materials_checked)
        ...
        if(is_collapsed)
            this.$el.find('.materials-data-body').find('input.li').prop('checked',true);
    },
    ...
    **/
    OnCollapse: function(e)
    {
        ...
        {
                 cur_btn.val("unCollapsed").html('&nbsp;&nbsp;Закрыть группы').prepend('<i class = "icon-folder-open"></i>');
                 if($(cur_btn).data('type')=="works")
                    this.$el.find('.works-data-body').find('input.li').prop('checked',true);
                else
                    this.$el.find('.materials-data-body').find('input.li').prop('checked',true);
        }
        else
        {
                 cur_btn.val("collapsed").html('&nbsp;&nbsp;Расскрыть группы').prepend('<i class = "icon-folder-close"></i>');
                 if($(cur_btn).data('type')=="works")
                    this.$el.find('.works-data-body').find('input.li').prop('checked',false);
                else
                    this.$el.find('.materials-data-body').find('input.li').prop('checked',false);
        }
    },
    ...
    **/
    OnVisible: function(e)
    {
        ...
        if(cur_btn.val()=="show")
        {
                 cur_btn.val("hide").html('&nbsp;&nbspСкрыть не выбранные').prepend('<i class = "fa fa-eye"></i>');
        }
        else
        {
                 cur_btn.val("show").html('&nbsp;&nbsp;Показать не выбранные').prepend('<i class = "fa fa-eye-slash"></i>');
                 // скрыть не выбранные
        }
        this.RenderMaterials(this.$el.find('.btn-collapse-materials').val()=='unCollapsed', this.$el.find('.btn-show').val() == 'show', (this.cur_work_item)?this.cur_work_item['work_materials']:null);
    },
    ...
    OnWorkItemClick: function(e)
    {
        ...
        for(var i in App.DataWorks)
        {
            if(App.DataWorks[i]['work_id']== cur_item_id)
            {
                ...
                {
                    this.RenderMaterials(this.$el.find('.btn-collapse-materials').val()=='unCollapsed', this.$el.find('.btn-show').val() == 'show', materials );
                }
        ...
        if(this.cur_work_item)
        {
            ...
            if($(cur_item).prop('checked'))
            {
                ...
                if(this.cur_work_item['work_materials'])
                    ...
                    this.cur_work_item['work_materials'] = [$(cur_item).data("id")]
                materials = this.cur_work_item['work_materials'];

                // формирование работ для материала
                if(cur_material_item['material_works'])
                    cur_material_item['material_works'].push(this.cur_work_item['work_id']);
                else
            ...
            else
            {
                ...
                if(this.cur_work_item['work_materials'])
                {
                    for(var i in this.cur_work_item['work_materials'])
                        if(this.cur_work_item['work_materials'][i]!=$(cur_item).data("id"))
                ...
                if(cur_material_item['material_works'])
                {
                    for(var i in cur_material_item['material_works'])
                        if(cur_material_item['material_works'][i]!=this.cur_work_item['work_id'])
            ...
            $.ajax({
                ...
                async:true
                        ...
                        {
                            $.jGrowl('Ошибка сохранения данных. Подробности: ' + result['msg'], { 'themeState':'growl-error', 'sticky':false });
                        }

                    }).fail(function(jqXHR, textStatus, errorThrown ) {
                            $.jGrowl('Ошибка сохранения данных. Подробности:' + errorThrown, { 'themeState':'growl-error', 'sticky':false });
                            Routine.hideLoader();
                    });
        }
    },
